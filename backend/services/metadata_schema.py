...
def chroma_metadata(metadata: dict[str, Any]) -> dict[str, Scalar]:
    ...
    for key, value in (
        ("path", asset_path(metadata)),
        ("thumbnail_path", thumbnail_path(metadata)),
        ("filename", filename(metadata)),
        ("mime_type", mime_type(metadata)),
        ("media_type", media_type(metadata)),
        ("content_hash", content_hash(metadata)),
        ("taken_sort", taken_sort(metadata)),
        ("taken_date", taken_date(metadata)),
        ("taken_year_month", taken_year_month(metadata)),
    ):
        if isinstance(value, (str, int, float, bool)):
            flat[key] = value
    ...
    location = capture.get("location") if isinstance(capture.get("location"), dict) else {}
    for source_key, target_key in (
        ("city", "geo_city"),
        ("state", "geo_state"),
        ("country", "geo_country"),
