...
import { getDurationSeconds, resolvedMediaUrl, resolvedThumbnailUrl } from "@/shared/media/mediaItem";
...
export function VideoDetailView({
  ...
  const [duration, setDuration] = useState(getDurationSeconds(item) ?? 0);
  ...
  useEffect(() => {
    setCurrentTime(0);
    setDuration(getDurationSeconds(item) ?? 0);
    setIsPlaying(false);
