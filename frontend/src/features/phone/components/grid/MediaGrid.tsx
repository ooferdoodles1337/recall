...
import { getAltText, getDurationSeconds, isAnimatedImage, isVideo, resolvedAnimatedThumbnailUrl, resolvedThumbnailUrl } from "@/shared/media/mediaItem";
import {
      ...
      >
        {thumb ? (
          <div className={`thumb-img-wrap${isBlurred ? " thumb-img-wrap--hidden" : ""}`}>
            <img src={thumb} alt={getAltText(result)} loading="lazy" decoding="async" onLoad={handleStaticThumbLoad} />
        ...
        {!isBlurred && video ? (
          <Badge variant="secondary" className="video-badge">
            <PlayIcon />
            {durationLabel(getDurationSeconds(result) ?? undefined) ?? "video"}
          </Badge>
        ) : !isBlurred && animated ? (
